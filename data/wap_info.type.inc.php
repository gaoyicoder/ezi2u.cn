...
$mymps_admin_info_type=array(
	...
	"checkbox"=>'
		<tr>
		<td bgcolor="#f5fbff" width="45%">
		<b>Option contents:</b><br />Valid only when the column can be selected. Options are placed one line at a time, with option indexes (application of number recommended) and option contents placed respectively in front of and behind the "=". Example: <br /><i>1=Apple<br />2=Banana<br />3=Pineapple</i><br />Note: Once an option is set, so is the relation between its index and content, so please do not make further changes to it. However, adding options is still possible. Should you wish to change the order of display of an option, move its line up or down.</td>
		<td bgcolor="#f5fbff">
	...
	"select"=>'
		...
		<b>Option contents:</b><br />Valid only when the column can be selected. Options are placed one line at a time, with option indexes (application of number recommended) and option contents placed respectively in front of and behind the "=". Example: <br /><i>1=Categorized Info System<br />2=Company-scale Website Building System<br />3=B2B Business System</i><br /><br />Note: Once an option is set, so is the relation between its index and content, so please do not make further changes to it. However, adding options is still possible. Should you wish to change the order of display of an option, move its line up or down.</td>
		<td bgcolor="#f5fbff">
	...
	if($forward == 'back'){
		switch($type){
			...
			case 'select':
				...
				//$mymps .="<label class=\"psu\">«Î—°‘Ò</label>";
			...
			break;
			case 'number':
				$required = $require == 1 ? "require=\"true\" datatype=\"limit|double\" msg=\"Please fill".$title."0 denotes discuss face to face" : "";
		...
		switch($type){
			...
			break;
			case 'number':
				$mymps .= "<input name=\"".$name."[min]\" type=\"text\"> to <input  name=\"".$name."[max]\" type=\"text\" float:none;\"> ";
			break;
		}
	}
	return $mymps;
