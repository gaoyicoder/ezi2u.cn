...
!ifplugin(CURSCRIPT) && exit('The coupon plug-in has either been disabled by the administrator or not been installed!');
...
if($id) {
	...
	if(!$coupon['id']) write_msg('This coupon has either expired or not yet passed the revision!','olmsg');
	...
	if($action == 'print'){
		...
		globalassign();
		include DIR_NAV.'/plugin/'.CURSCRIPT.'/template/print.tpl.php';
	...
	foreach(array('hit'=>'Default','prints'=>'Print','dateline'=>'Newest') as $k => $v){
	...
	$currentlocate = $areaname.$couponclass[$cate_id]['cate_name'].' Coupons';
...
function get_coupon_location($cate_id=0,$str=''){
	...
	$location   = 'Current Position: <a href="'.$mymps_global['SiteUrl'].'">'.$city['cityname'].$GLOBALS['mymps_global']['SiteName'].'</a>'.' <code>'.$raquo.'</code> '.' <a href="'.plugin_url(CURSCRIPT,array('cate_id'=>0)).'">'.$city[cityname].' Coupons</a>';
	$page_title = $city['cityname'].' Coupons - '.$mymps_global['SiteName'];
