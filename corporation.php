...
if($catid && !$cur = $db -> getRow("SELECT parentid,corpname FROM `{$db_mymps}corp` WHERE corpid = '$catid'")) write_msg('This seller category either does not exist or is yet to be enabled!');
...
$level_limit	= " AND levelid > '1' AND if_corp = '1'";
...
if(is_array($res = page1($sql,$mymps_global['cfg_list_page_line'] ? $mymps_global['cfg_list_page_line'] : 10))){
	foreach($res as $key => $val){
		...
		$arr['address']		= $val['address'] ? $val['address'] : ($val['busway'] ? $val['busway'] : 'No Address For Now'); 
		$arr['levelid']		= $val['levelid'];
		$arr['tel']			= $val['tel'] ? $val['tel'] : 'No Phone Number For Now';
		$member[]			= $arr;
	}
}
...
if($city['cityid']){
	...
	if(is_array($area_list)){
		$area_list	= array_merge(array('0'=>array('areaid'=>'','areaname'=>'Do Not Limit District')),$area_list);
		if(is_array($area_list)){
...
$hotmember		= $cityid ? mymps_get_member_list('','','',1,'','',2) : mymps_get_member_list(10,'','',1,'','',2);
$pageview = page2($rewrite);
$certifymember = mymps_get_member_list(10,1,'',1,1);
$totalnum = $rows_num;
