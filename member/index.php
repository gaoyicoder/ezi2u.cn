...
if($if_corp != 1 && in_array($m,array('goods','coupon','group','document','certifycorp','album','comment'))) write_msg('You are not a seller member. Please apply for an online shop before continuing!','index.php?m=shop');
...
function location($type='user',$str=''){
	...
	$return = '<a href="index.php"><span>Member Centre</span></a> '.$raquo;
	$return .= $str != '' ?  ' <a href="index.php?m='.$m.'">'.($member_menu[$type][$m] != "" ? $member_menu[$type][$m] : "Homepage").'</a> '.$raquo.' <span>'.$str.'</span>' : ' '.($member_menu[$type][$m] != "" ? $member_menu[$type][$m] : "Homepage");
	return $return;
	unset($raquo,$return);
}
exit;
