...
<div class="footer">
    <div class="clearfix footer-inner">
        <p class="copyright">
            CopyRight &copy; <a href="<?php echo $mymps_global['SiteUrl']; ?>"><?php echo $mymps_global['SiteName']; ?></a> Contact Hotline: <?php echo $mymps_global['SiteTel']; ?>
        </p>
        <p class="extrainfo">
...
<script type="text/javascript">
    ...
    var resultdivs  = $('msg_none').getElementsByTagName('div');
    var resultobj  = null;
	for(var i = 0; i < resultdivs.length; i++) {
        if(resultdivs[i].id.substr(0, 4) == 'msg_' && (resultobj = $(resultdivs[i].id.substr(0, resultdivs[i].id.length - 5))) && resultdivs[i].innerHTML) {
			resultobj.innerHTML = resultdivs[i].innerHTML;
			resultobj.className = resultdivs[i].className;
		}
	}
	setTimeout('hiddendiv()',6000);//3秒
</script>
<?php 
