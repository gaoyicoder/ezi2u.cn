...
if($ac == 'del'){
	...
	$r = $db -> getOne("SELECT a.* FROM `{$db_mymps}information` AS a {$wherea} AND a.id =".$id);
	...
	if($info[modid] > 1) mymps_delete("information_".$info[modid],"WHERE id = '$id'");
	...
	write_money_use("Post numbered $id has been <font color=red>refreshed</font> successfully","<font color=red>coins deducted ".$delmoney." </font>");
	...
	write_money_use("Post numbered $id has been <font color=red>reddened on title</font> successfully","<font color=red>coins deducted ".$delmoney." </font>");
	...
	write_money_use("Post numbered $id has been <font color=red>overstruck on title</font> successfully","<font color=red>coins deducted ".$delmoney." </font>");
	...
	$location = location('user','Top Post');
	...
	if(empty($id) || empty($catid) || empty($upgrade_time) || !in_array($upgrade_type,array('cfg_member_upgrade_index_top','cfg_member_upgrade_top','cfg_member_upgrade_list_top'))) {
		...
		if($box) {
			...
			write_msg('Topping failed, for the title ID of the post to be topped is empty or category to be topped in is incorrect');
	...
	if($money_own < $money_cost) {
		...
		if($box) {
			...
			write_msg('You don��t have enough scores in your account, please recharge before continuing','member/index.php?m=pay&box=1','',1);
	...
	if($upgrade_type == 'cfg_member_upgrade_top') {
		...
		if($box){
			...
			$iftop == 2 && write_msg('This post title has already been placed at the top of the broad headings');
		...
		if($box){
			...
			$iflisttop == 2 && write_msg('This post title has already been placed at the top of the sub-headings');
		...
		if($box){
			...
			$ifindextop == 2 && write_msg('This post title has already been placed at the top of the homepage');
		...
		if(mymps_count("information","WHERE upgrade_type_index = '2'") > 12){
			...
			if($box){
				...
				write_msg('Topping failed, for there are not enough slots on homepage. Please try again tomorrow');
	...
	$caozuo = $upgrade_type == 'cfg_member_upgrade_index_top' ? 'Place at the Top of the Homepage' : ($upgrade_type == 'cfg_member_upgrade_top' ? 'Place at the Top of the Broad Headings' : 'Place at the Top of the Sub-headings');

	write_money_use("Post numbered $id has been <font color=red>".$caozuo."</font> successfully","<font color=red>coins deducted ".$money_cost." </font>");
	...
	if($upgrade_type == 'cfg_member_upgrade_top'){
		...
		$do_action = 'Place at the Top of the Broad Headings';

	}elseif($upgrade_type == 'cfg_member_upgrade_list_top'){

		$do_action = 'Place at the Top of the Sub-headings';
	...
	if($box){
		...
		write_msg('Congratulations, this post title has been<font color=red>'.$caozuo.'</font>��<br /><br /><input value="Close Window" type="button" onclick=\'parent.window.location.reload();parent.closeopendiv();\' style="margin-left:auto;margin-right:auto;" class="blue">','olmsg');
