...
<div id="<?=MPS_SOFTNAME?>">
    ...
    <td colspan="6"><b><?=$certify_arr[$typeid]?>Authentication Submission Record</b></td>
...
javascript:setbg('MympsMember Centre',400,110,'../box.php?part=member&userid=<?=$value[userid]?>&admindir=<?=$admindir?>')"><?=$value[userid]?></a>

<hr style="width:80px;"/>

<?php if($value[per_certify] == '1'){?>

        <img src="../images/person1.gif" alt="Identity Verification Passed"/>

        <?}else{?>

        <img src="../images/person0.gif" alt="Identity Verification Failed"/>
        ...
        <?}?>
          ...
          <td><a href="?part=yes&userid=<?=$value[userid]?>&typeid=<?=$typeid?>&page=<?=$page?>">Verification Passed</a> | <a href="?part=no&userid=<?=$value[userid]?>&typeid=<?=$typeid?>&page=<?=$page?>">Verification Failed</a></td>
...
<input type="submit" value="Submit" class="mymps large" name="certification_submit"/>  
