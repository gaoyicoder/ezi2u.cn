...
<div class="mpstopic-category">
	<div class="panel-tab">
		<ul class="clearfix tab-list">
			...
			<li><a href="?part=qqlogin" <?php if($part == 'qqlogin'){?>class="current"<?php }?>>QQ登录整合 (中国元素)</a></li>
		</ul>
	</div>
</div>
</div>
...
<table border="0" cellspacing="0" cellpadding="0" class="vbm">
  ...
  <tr bgcolor="#ffffff">
    <td id="menu_tip">
<li><?php if($part == 'bbs'){?>If a third party system is to be Integrate d into the system, please make sure the configuration is done correctly, otherwise users may not be able to register or log-in Mymps successfully!<?php }else {?>在你开通QQ帐号(中国元素,建议斟酌后再确定是否保留)登录整合之前请到 <a href="http://opensns.qq.com/login?from=http://connect.opensns.qq.com/apply" target="_blank" style="text-decoration:underline">http://opensns.qq.com/login?from=http://connect.opensns.qq.com/apply</a>Apply to Obtain APPID, APPKEY and CALLBACK Address<?}?></li>
    </td>
  </tr>
</table>
</div>
...
<div id="<?=MPS_SOFTNAME?>">
<table border="0" cellspacing="0" cellpadding="0" class="vbm">
<tr class="firstr"><td colspan="2">配置QQ登录参数(中国元素)</td></tr>
<tr bgcolor="#ffffff" style="font-weight:bold">
<td width="25%" style=" text-align:right; background-color:#f7f7f7">
是否开启QQ登录整合(中国元素)：
</td>
<td style="background-color:#f7f7f7">
<label for="open"><input name="qqsettings[open]" type="radio" class="radio" id="open" value="1" onclick='$("qqdetail").style.display = "";' <?php if($qqsettings['open'] == 1){echo 'checked';}?>>开启</label> 
<label for="close"><input class="radio"  name="qqsettings[open]" type="radio" id="close" value="0" onclick='$("qqdetail").style.display = "none";' <?php if(!$qqsettings['open']){echo 'checked';}?>>关闭</label>
</td>
</tr>
...
<tr bgcolor=#FFFFFF>
	<td height=25><div align="right">Callback Address: </div><div class="ttip">QQ登录成功后跳转的地址，请确保地址真实可用，否则会导致登录失败。(中国元素)</div></td>
    <td>      
