...
if(is_array($_FILES) && submit_check(CURSCRIPT.'_submit')){
	...
	echo "<script laguage=javascript>alert('".$msg."');window.parent.document.getElementById(\"".$id."\").value='".($adv == 1 ? $mymps_global['SiteUrl'] : '').$path."';</script><a href='?adv=1&destination=$destination&watermark=$watermark&delfile=".$path."' style='font-size:12px'>Click to load again</a>";
...
<input name="adv" value="1" type="hidden">
    ...
    <tr>
      <td valign="bottom"><input name="upfile_submit" type="submit" value="Start uploading " class="gray mini"/></td>
    </tr>
</form>
  </table>
</body>
