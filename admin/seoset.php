...
if($seo['seo_force_category'] == 'rewrite_py'){
	$conf .='
RewriteRule ^(.*)/(?!\m\b|'.$admdir.')([^\/]+)/$ $1/category\.php\?Catid=$2';
...
';
	...
	if(!createfile(MYMPS_ROOT.'/apache.txt',$conf)){
		write_msg(MYMPS_ROOT.'The file /apache.txt cannot be written. Please check your authority to operate root files!');
	} else {
		write_msg('The file apache.txt has successfully been updated!','seoset.php');
	}
	unset($conf,$cities);
}
...
if(!submit_check(CURSCRIPT.'_submit')){
	...
	$here = MPS_SOFTNAME.'SEO Optimization Settings';
	
	chk_admin_purview("purview_SEO Pseudo");
	...
	if($updatefile == 1){
		...
		if(!createfile(MYMPS_ROOT.'/rewrite/httpd.ini',$rules['iis'])){
			$notice .= MYMPS_ROOT.'/rewrite/httpd.ini 请设置为777属性或者写入修改权限（不是非常懂）<br><br>';
		}
		if(!createfile(MYMPS_ROOT.'/rewrite/.htaccess',$rules['apache'])){
			$notice .= MYMPS_ROOT.'/rewrite/.htaccess 请设置为777属性或者写入修改权限（不是非常懂）<br><br>';
		}
		if(!createfile(MYMPS_ROOT.'/rewrite/web.config',$rules['iis7'])){
			$notice .= MYMPS_ROOT.'/rewrite/web.config 请设置为777属性或者写入修改权限（不是非常懂）<br><br>';
		}
		if(!createfile(MYMPS_ROOT.'/rewrite/nginx.conf',$rules['nginx'])){
			$notice .= MYMPS_ROOT.'/rewrite/nginx.conf 请设置为777属性或者写入修改权限（不是非常懂）<br><br>';
		}
	}
	
	write_msg(($notice ? $notice : '').'System SEO Settings have successfully been updated!','seoset.php','WriteRecord');
