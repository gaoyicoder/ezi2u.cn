...
if($part != 'phpinfo'){
	...
	chk_admin_purview("purview_System");
}
...
elseif($part == 'list'){
	...
	$here = "Ezi2u System Parameter Settings";
...
elseif($part == 'update'){
	...
	update_config_cache();
	unset($admin_global);
	write_msg ("System parameter has successfully been set!",CURSCRIPT.".php","record");
}
/*
词语过滤设置
*/
elseif($part == 'badwords'){
	chk_admin_purview("purview_Verify Filtered");
	if(trim($action) == 'dopost'){
		...
		write_msg ("Words and phrases filtering settings have successfully been done!","config.php?part=badwords","write_record");
	}else{
		$here = 'Forbidden Content Filtering Settings';
...
elseif($part == 'commentsettings'){
	if($action == 'do_post'){
		...
		write_msg('Comments and remarks settings have successfully been updated!','config.php?part=commentsettings','WriteRecord');
	}else{
		$here = 'Comments And Remarks Settings';
...
elseif($part == 'cache_sys'){
	chk_admin_purview("purview_Cache");
	$here = '数据缓存';
	$cachearray = array(
				'write_admin_cache'=>'Update Administrators Cache',
				'updateadvertisement'=>'Update Advertisements Cache',
				'write_cron_cache'=>'Update Timed Task Cache',
				'write_checkanswer_cache'=>'Update Identifying Question And Answer Cache',
				'update_checkanswer_settings'=>'Update Identifying Question And Answer Settings Cache',
				'update_jswizard_settings'=>'Update JS Data Invocation Settings Cache',
				'write_jswizard_cache'=>'Update JS Data Invocation Cache',
				'write_authcode_cache'=>'Update Identifying Code Settings Cache',
				'write_plugin_cache'=>'Update Plug-in Cache',
				'write_insidelink_cache'=>'Update In-site text Link Cache'
				);
...
elseif($part == 'cache_sysupdate'){
	...
	$updatecache = NULL;
	write_msg('System cache has successfully been updated with '.$count.'cache files deleted!',$return_url ? $return_url : '?part=cache_sys');
}
/*
更新前台缓存
*/
elseif($part == 'cacheupdate'){
	...
	write_msg("System cache has successfully been set!","config.php?part=cache","record");
}
/*
验证码设置
*/
elseif($part == 'imgcode'){
	...
	if(trim($action) == 'do_post'){
		...
		foreach(array('noise','line','snow') as $key){
			$db -> query("INSERT INTO `{$db_mymps}config` (description,value,type) values ('$key','$settingsnew[$key]','authcode')");
		}
		
		write_authcode_cache();
		write_msg('Identifying code settings has successfully been updated!','?part=imgcode','write_record');

	}else{
		$here = "Identifying Code Display Settings";
...
elseif($part == 'checkask'){
	...
	if(trim($action) == 'do_post'){
		...
		write_msg('Identifying question and answer settings has successfully been updated! ','?part=checkask','write_record');
		
	}else{
		$here = "Identifying Question And Answer Settings";
