...
<div class="body1000">
	...
	<div class="wrapper">
		<div class="information_bd <? if($info[info_level] == 2){ ?>tuijian<? }?>">
			<div class="bd_left">
				<div class="information_hd">
					<ul>
						...
						<div class="information_time">
							<span class="viewhits">Views: <font id="hit" color="red">
							<script type="text/javascript" src="<?=$mymps_global[SiteUrl]?>/javascript.php?part=information&id=<?=$info[id]?>"></script>
							</font></span><span class="lasttime">This Post<?=$info[endtime]?></span><span class="begintime">Time Of Post: <?=GetTime($info[begintime])?></span>
						</div>
					</ul>
				</div>
				...
				<div class="view_bd">
					...
					<?if($info[image]){?>
					<p>
						<? foreach($info[image] as $k => $v){?> <img src="<?=$mymps_global[SiteUrl]?><?=$v[path]?>" class="imginfo" title="<?=$v[title]?>" alt="<?=$v[title]?>"><br>
						<? }?>
					</p>
					<div class="clear">
					</div>
					...
					<ul>
						<? if($info[contactview] == 1){?>
							...
							<? }?>
							<li><span>Contact: </span><?=$info[contact_who]?> <? if($info[ip2area] == 'wap'){?><font class="font" color="green">Post Via Mobile Phone</font><? }else{?><font class="font"><?=$info[ip]?></font> <font class="font" color="green"><?=$info[ip2area]?></font><? }?></li>
							...
							<? }else{?>
								<li><span>Contact Phone: </span><font class="tel"><?=$info[telephone]?></font> <font class="font"><a rel="nofollow" href="<?=$info[posthistory]?>" target="_blank">View Posting Records</a></font></li>
								<? if($info[qq]){?><li><!--<span>Facebook:</span>--><?=$info[qq]?></li><? }?>
								<? if($info[email]){?><li><span>Email: </span><?=$info[email]?></li><? }?>
							<? }?>
							<li><span>Reminder: </span>Please remind me that you saw it on <?=$mymps_global[SiteName]?> when contacting me, thank you!</div></li>
		...
		<!---XXXXXXXXXXXXXXXXXXXXXXX-->
			...
			<div class="bd_right">
				<div class="boxer_hd cfix">
					Manage Posts<font color="#999999">（Number: <?=$info[id]?>）</font>
				</div>
				<div class="boxer_bd cfix action">
					<ul>
						<li><a href="javascript:void(0);" onclick="setbg('Top A Post',538,248,'<?=$mymps_global[SiteUrl]?>/box.php?part=upgrade&id=<?=$info[id]?>');" style="color:red">Top</a></li>
						<li><a rel="nofollow" href="javascript:void(0);" onclick="setbg('Save Post To Favourites',538,270,'<?=$mymps_global[SiteUrl]?>/box.php?part=shoucang&infoid=<?=$info[id]?>')">Favourite</a></li>
						<li><a rel="nofollow" href="javascript:void(0);" onclick="setbg('Delete Post',538,270,'<?=$mymps_global[SiteUrl]?>/box.php?part=delinfo&id=<?=$info[id]?>')" title="You cannot restore a deleted item!">Delete</a></li>
						<li><a rel="nofollow" href="<?=$mymps_global[SiteUrl]?>/<?=$mymps_global[cfg_postfile]?>?action=edit&id=<?=$info[id]?>" target="_blank">Edit</a></li>
						<li><a class="report" href="javascript:void(0);" onclick="setbg('Report Post',470,270,'<?=$mymps_global[SiteUrl]?>/box.php?part=report&id=<?=$info[id]?>&infotitle=<?=$info[title]?>');">Report</a></li>
					</ul>
				</div>
				<div class="boxer_hd cfix">
					Your Interested Posts
				</div>
				<div class="boxer_bd cfix">
					<ul>
						<? foreach($latest_info as $k => $v){?>
						<li><a style="<? if($v[ifred] == 1){?>color:red;<? }?><? if($v[ifbold] == 1 ){?>font-weight:700;<? }?>" href="<?=$v[uri]?>" target="_blank" title="<?=$v[title]?>"><? if($v[img_path]){?><font color="green">[Pic]</font> <? }?><?=$v[title]?></a></li>
						<? }?>
					</ul>
				</div>
				<div class="boxer_hd cfix">
				...
				<div class="boxer_bd cfix noborder short">
					<ul>
						<? foreach($relate_cat as $k => $v){?> <? foreach($v[children] as $u => $w){?>
						<li><a href="<?=$city[domain]?><?=$w[uri]?>" title="<?=$city[cityname]?> <?=$w[catname]?>"> <?=$w[catname]?></a></li>
						<? }?> <? }?>
					</ul>
				</div>
				<div class="boxer_hd cfix">
					Popular State <?=$info[catname]?> Information
				</div>
				<div class="boxer_bd cfix noborder short">
					<ul>
