	...
	<div class="body1000">
		...
		<div class="infolists">
			...
			<ul>
				...
				<a href="<?=$v[uri]?>" target="_blank" style="<? if($v[ifred] == 1){?>color:red;<? }?> <? if($v[ifbold] == 1){?>font-weight:bold;<? }?>"><?=$v[title]?></a><span class='highlight'>[<?=$v[position]?>]</span><? if($v[img_count]){?><span class="img_count"><?=$v[img_count]?>Image</span><? }?><? if($v[info_level] == 2){?><span class="tuijian">Recommended</span><? }?><? if($v[certify] == 1){?><span class="certify">Verified</span><? }?>&nbsp;<br /><font style="font-size:12px; color:#808080"><?=cutstr($v[content],70)?></font><br />
				<font style="font-size:12px"><?=$v[areaname]?> / <?=$v[rent_type]?> </font>
				</span>
				<span class="ltime"><?=$v[begintime]?></span>
				<span class="lprice"><span class="rprice"><? if($v[mini_rent] == '0元（中国元素，建议斟酌）'){?>Discuss Face To Face<? }else{?><?=$v[mini_rent]?><? }?></span><br /><?=$v[room_type]?></span>
				</div>
				<? }?>
			</ul>
			<!--栏目列表页尾部广告开始-->
			...
			<div class="clear"></div>
			<div class="totalpost"><a href="<?=$mymps_global[SiteUrl]?>/<?=$mymps_global[cfg_postfile]?>?catid=<?=$cat[catid]?>&cityid=<?=$city[cityid]?>" target="_blank">Make A Post On <?=$mymps_global[SiteCity]?><?=$cat[catname]?> Now&raquo;</a></div>
		</div>
	</div>
	<div class="clear"></div>
