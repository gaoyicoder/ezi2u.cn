...
<div class="body1000">
	...
	<div class="infolists">
		...
		<span class="lsex"><?=$v[sex]?>&nbsp;</span>
		<span class="ltitlevalue"><br />
		<a href="<?=$info[uri]?>" target="_blank" style="<? if($v[ifred] == 1){?>color:red;<? }?> <? if($v[ifbold] == 1){?>font-weight:bold;<? }?>"><?=$v[title]?></a><? if($v[img_count]){?><span class="img_count"><?=$v[img_count]?>Image</span><? }?><? if($v[certify] == 1){?><span class="certify">Verified</span><? }?><font class="area">(<?=$v[areaname]?>)</font> <br />
		<?=$v[age]?> / <?=$v[jobs]?>
		</span>
		...
		<div class="clear"></div>
		<div class="totalpost"><a href="<?=$mymps_global[SiteUrl]?>/<?=$mymps_global[cfg_postfile]?>?catid=<?=$cat[catid]?>" target="_blank">Make A Post On <?=$city[cityname]?> <?=$cat[catname]?>   Now&raquo;</a></div>
	</div>
	</div>
	<div class="clear"></div>
