	...
	<div class="body1000">
		...
		<div class="infolists">
			...
			<ul class='sep'>
				...
				<div class='hover media cfix <? if($v[upgrade_type] > 1){?>ding<? }?>'>
				<a href='<?=$v[uri]?>' target='_blank' class='media-cap'><img src='<? if($v[img_path] == ''){?><?=$mymps_global[SiteUrl]?>/images/nophoto.gif<? }else{?><?=$mymps_global[SiteUrl]?><?=$v[img_path]?><? }?>' title='<?=$v[title]?>'></a>
				<div class='media-body'>
				<div class='media-body-title'>
				<span class='pull-rights'><span class="highlights"><? if($info[price] == '0元（中国元素，建议斟酌）'){?> Discuss Face To Face<? }else{?><?=$v[price]?><? }?></span></span>
				<a href="<?=$v[uri]?>" target="_blank" style="<? if($v[ifred] == 1){?>color:red;<? }?> <? if($v[ifbold] == 1){?>font-weight:bold;<? }?>">[<?=$v[new_old]?>]<?=$v[title]?></a><? if($v[img_count]){?><span class="img_count"><?=$v[img_count]?>Image</span><? }?><? if($v[info_level] == 2){ ?><span class="tuijian">Recommended</span><? }?><? if($v[certify] == 1){?><span class="certify">Verified</span><? }?><br /><font color="#808080"><?=cutstr($v[content],100)?></font>
				</div>
				<div class='typo-smalls'><?=$v[areaname]?> / <?=$v[begintime]?></span> <span class='highlight'><?=$v[from]?></span></div>
				</div>
				</div>
				<? }?>
				<!--栏目列表页尾部广告开始-->
