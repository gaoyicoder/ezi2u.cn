...
if($id) {
	...
	if(!$news = $db -> getRow("SELECT * FROM `{$db_mymps}news` WHERE id = '$id'")){
		...
		write_msg('Your designated news either does not exist or has been deleted!');//�����������

		exit;

	}

	$latest_news = mymps_get_news_list(15);//ȡ�����·�����10������

	$relate_news = mymps_get_news_list(8,$news['catid']);//ȡ����ص�8������

	$image_news = mymps_get_news_list(6,'',1);//ȡ�����µ�6��ͼ������
	...
	$catid = isset($catid) ? intval($catid) : 0;//��ʼ��catid

	$channel = get_cat_info($catid,'channel');

	if(!$channel) write_msg('Your designated news column either does not exist or has been deleted!');//��õ�ǰ��Ŀ�������Ϣ

	$loc		= get_location('channel',$catid);//���ҳ�����
	...
	$cat_list = get_categories_tree(empty($channel['parentid']) ? $catid : $channel['parentid'],'channel');//���ͬ����Ŀ

	$latest_info = mymps_get_info_list(10,'','','','','','','',false);

	$latest_news = mymps_get_news_list(15,false);//ȡ�����·�����10������

	$image_news = mymps_get_news_list(6,'',1,false);//ȡ�����µ�6��ͼ������;
	...
	$top_news	= mymps_get_news_list(18,'',0);//ȡ�����µ�18������

	$hot_news	= mymps_get_news_list(20,'',0,false,1);//ȡ�����µ��Ƽ�����
