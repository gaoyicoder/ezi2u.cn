...
function check_upimage($file="filename")
{
	...
	if($_FILES[$file]['size']>$size){
		write_msg('Images to upload should be no more than '.$mymps_global['cfg_upimg_size'].'KB in size!');
		exit;
	}
	
	if(!in_array(FileExt($_FILES[$file]['name']),$upimg_allow)){
		write_msg('Only images of '.$mymps_global['cfg_upimg_type'].'formats are recognized by the system!');
	}
	
	if(!preg_match('/^image\//i',$_FILES[$file]['type'])){
		write_msg ('Sorry, the system cannot recognize the format of the uploaded image. Please try and upload another image!');
	}
	return true;
}
...
function start_upload($file_name,$destination_folder,$watermark =0,$limit_width='',$limit_height='',$edit_filename='',$edit_pre_filename=''){
	...
	!is_uploaded_file($_FILES[$file_name]['tmp_name']) && write_msg ("Please reselect the image to upload!");
    ...
    if (file_exists($destination)){
        write_msg("An image with the same name already exists. Please reselect the image to upload!");
    }
	
    if(!move_uploaded_file($file_name, $destination)){
        write_msg("Image uploading failed. Please reselect the image to upload!");
    }
