...
if(!$member_log->chk_in()) alert("Please log in as a member before uploading any image in the member centre!");
...
if (empty($_FILES) === false) {
	...
	if($_FILES[$name_file]['size']>$size){
		alert('Each file to be uploaded should be smaller than'.$mymps_global['cfg_upimg_size'].'KB');
	}
	
	if(!in_array(FileExt($_FILES[$name_file]['name']),$upimg_allow)){
		alert('Only'.$mymps_global['cfg_upimg_type'].'images are allowed to upload by the system!');
	}
	
	if(!preg_match('/^image\//i',$_FILES[$name_file]['type'])){
		alert('Apologies! The system can not recognize the type of the image uploaded, please upload another image.');
	}
