...
class mymps_member_log
{
	...
	function PutLogin($s_uid='',$s_pwd='',$memory='')
	{
		global $timestamp;
		if ($memory == "on"){
			setcookie('s_uid',$s_uid, $timestamp+3600*24*30 , '/');
			setcookie('s_pwd',mmd5($s_pwd,'EN','',$this->cookiepre), $timestamp+3600*24*30,'/');
		}else{
			setcookie('s_uid', $s_uid ,"0" , "/");
			setcookie('s_pwd', mmd5($s_pwd,'EN','',$this->cookiepre) ,"0" , "/");
		}
	}
	...
	function out($url='')
	{
		...
		$s_uid = isset($_COOKIE['s_uid']) ? addslashes($_COOKIE['s_uid']) : '';
		$timestamp = $timestamp ? $timestamp : time();
	...
	function chk_in()
	{
		...
		$s_uid = isset($_COOKIE['s_uid']) ? addslashes($_COOKIE['s_uid']) : '';
		$s_pwd = isset($_COOKIE['s_pwd']) ? $_COOKIE['s_pwd'] : '';
		if(empty($s_uid)){
	...
	function get_info()
	{
		...
		$s_uid=addslashes($_COOKIE['s_uid']);
