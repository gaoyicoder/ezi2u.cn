...
function msetcookie($var,$val,$life=0){
	...
	return setcookie($cookie_pre.$var,$val,$life,$cookie_path,$cookiedomain);
}
...
function get_editor($editor_name,$type,$value = '',$width='100%',$height='400px',$BasePath='../include/kindeditor')
{
	...
	$html .= '<textarea require="true" datatype="limit" msg="Please Enter Post Content Description" msgid="content" name="'.$editor_name.'" style="width:'.$width.';height:'.$height.';visibility:hidden;">'.$value.'</textarea>';  
...
function mymps_chk_randcode($getcode='')
{
	...
	$sessioncode = $_SESSION['chkcode'];
	if($sessioncode != $getcode){
		return false;
	}else{
		return true;
	}
...
function CheckUserID($uid,$msgtitle='User ID')
{
	for($i=0;isset($uid[$i]);$i++){
		if(ord($ck_uid[$i]) > 0x80){
			if(isset($uid[$i+1]) && ord($uid[$i+1])>0x40){
				...
				return $msgtitle.'We strongly recommend that you use a combination of English letters and digits!';
			}
		}
	}
	return 'ok';
	...
	if(defined('IN_AJAX')){
		...
		if (!empty($msg)&&!empty($url)){
			if($url=="javascript:history.go(-1);"){
				...
				$goto = "<a href='".$url."'>If your browser doesn't respond, please click here...</a>";
			}elseif($url == "olmsg"){
				$time_echo = $goto = "";
			}else{
...
function show_msg($msgs,$ms="")
{
	...
	$title = $mymps_global[SiteName].'Notice';
...
function is_member_info($id,$type='public')
{
	global $db,$db_mymps;
	if(empty($id)){
		write_msg("Operation failed for you have not selected any subject!");
	}
...
function globalassign(){
	global $mymps_global,$navigation,$navurl_head,$navurl_foot,$docunav,$about,$city;
	...
	if(CURSCRIPT == 'store'){
		...
		if(!in_array(CURSCRIPT,array('login','post','about'))){
			$navigation = mymps_get_navurl('header',15);//获得头部主导航
			$navurl_head = $city['topnav'];//获得头部副导航
			$navurl_foot = mymps_get_navurl('foot',30);//获得尾部链接导航
		}
	}
}
...
function mymps_tpl($str,$smarty='')
{
	...
	if (defined('IN_SMT')){
		if(CURSCRIPT == 'search'){
			return MYMPS_TPL.'/search/'.$str.'.tpl.php';
		}elseif(CURSCRIPT == 'store'){
			$GLOBALS[store][template] = $GLOBALS['store']['template'] ? $GLOBALS['store']['template'] : 'blue';
			return MYMPS_TPL.'/spaces/store/'.$GLOBALS['store']['template'].'/'.$str.'.tpl.php';
		}else{
			return MYMPS_TPL.'/default/'.$str.'.tpl.php';
		}
	}else{
...
{
	$msgs="Unknown error! This may be in relation to the parameter you submitted. <br /><br />To get further help, please go to <a href=http://mymps.com.cn target=_blank>Mymps official site.</a>";
...
function get_info_life_time($time)
{
	...
	if($last_time >= 5){
		$last_time = "&nbsp;<font color=green>$last_time</font> &nbsp;days overdue";
	}elseif($last_time > 0 && $lastime < 5){
		$last_time = "&nbsp;<font color=red>$last_time</font> &nbsp;days overdue";
	}elseif($last_time == 0){
		$last_time = '<font color=green>&nbsp;Long-term effective</font>';
	}else{
		$last_time = '<font color=red>&nbsp;Expired</font>';
	}
	return $last_time;
}
...
/*百宝箱*/
function get_lifebox($num='12'){
	$data = read_static_cache('lifebox');
	if($data === false){
		$num = $num ? " LIMIT 0,$num" : "";
		$res = $GLOBALS['db'] -> getAll("SELECT * FROM `{$GLOBALS['db_mymps']}lifebox` WHERE if_view = 2 ORDER BY displayorder DESC $num");
		write_static_cache('lifebox',$res);
	} else {
		$res = $data;
	}
...
/*便民电话*/
function get_telephone($num='20'){
	$data = read_static_cache('telephone');
	if($data === false){
		$nu = $num ? " LIMIT 0,$num" : "";
		$res = $GLOBALS['db'] -> getAll("SELECT * FROM `{$GLOBALS['db_mymps']}telephone` WHERE if_view = 2 ORDER BY displayorder DESC $nu");
		write_static_cache('telephone',$res);
	} else {
		$res = $data;
	}
	return count($res)>0?$res:false;
...
function get_location($type = 'category',$cat = 0, $str = '', $extra='',$pdetail='')
{
	...
	if(in_array($type,array('channel','news'))){
		$location   = 'Current position：<a href="'.$mymps_global[SiteUrl].' ">'.$mymps_global['SiteName'].'</a>';
	}else{
		$location   = 'Current position：<a href="'.$city[domain].' ">'.$city['cityname'].' '.$mymps_global['SiteName'].'</a>';
	}
	...
	if(in_array($type,array('category','channel','corp','news'))){
		...
		if($type == 'channel'){
			$location .= ' <code>'.$raquo.'</code> <a href="'.Rewrite('news',array('action'=>'index')).'">'.$city[cityname].'Site News</a>';
			$page_title = $city['cityname'].'Site News - '.$page_title;
		} elseif($type == 'corp') {
			$location .= ' <code>'.$raquo.'</code> <a href="'.Rewrite('corp',array('action'=>'index')).'">'.$city[cityname].'Stores</a>';
			$page_title = $city[cityname].'Stores - '.$page_title;
		}
    ...
    /* 处理最后一部分 */
    if (!empty($str)){
        $page_title = $str.($type == 'space' ? 'Personal Space' : '').' - '.$page_title;
        $location   .= ' <code>'.$raquo.'</code> &nbsp;' .$str.($type == 'space' ? 'Personal Space' : '');
    }
...
function get_upload_image_view($if_upimg = 1 , $infoid = '')
{
	...
	if($if_upimg == 1){
		...
		for($i=0;$i<$cfg_upimg_number;$i++){
			$mymps .= $i==0 ? '' : '<br />';
			$mymps .= '<input class="input" type="file" name=mymps_img_'.$i.' datatype="filter" msg="The Image File Is In The Wrong Format">';
			$mymps .= '<br />';
		}
	}
	return $mymps;
...
function get_upload_image_edit($if_upimg = 1,$infoid,$admin='no'){
	...
	if($if_upimg == 1){
		...
		if($admin == 'no'){
			$mymps .='<tr><td class=tdr>Upload Image：</td><td class="upload_img">';
		}else{
		...
		for($i=0;$i<$cfg_upimg_number;$i++){
			...
			<li style="text-align:center; font-weight:100; color:#000; font-size:12px; line-height:22px"><input name="delinfoimg['.$i.']" type="checkbox" class="checkbox"> <font>Delete?</font></li>
			</ul>
			';
		}
		$mymps .= '</td></tr>';
...
function get_format_time($time){
	...
	if($limit<60){
		$strTime=$limit.'sec before';	
	}else if($limit/60<60){
		$strTime=floor($limit/60).'min before';
	}else if($limit/60/60<24){
		$strTime=floor($limit/60/60).'hr(s) before';
	}else if($limit/60/60>24&&$limit/60/60<48){
		$strTime='1 day ago'.date('H:i',$time);
	}else if($limit/60/60/24>2 && $limit/60/60/24<3){
		$strTime='2 days ago'.date('H:i',$time);	
	}else if($limit/60/60/24<7){
		$strTime=floor($limit/60/60/24).' days ago';
	}else{
...
function get_sex_option($cursex=''){
	foreach (array('Male','Female') as $key){
...
function get_htmlpath_type($dir_type,$typename,$id,$mydir){
	...
	if($dir_type == 1){
		...
		//自定义栏目名
		!$mydir && write_msg('Please enter your customized column name!');
		$html_path = $mydir;
	} else {
		$html_path = false;
	}
...
function get_info_option_titval($option,$value){
	global $charset;
		...
		while($row = $GLOBALS['db'] -> fetchRow($query)){
			...
			foreach($option as $w => $u){
				...
				if($nrow['search'] == 'on'){
					...
					if(in_array($nrow['type'],array('select','radio','number')))
					{					
						if(is_array($extra)){
							foreach($extra as $k => $value){
								if($nrow[type] == 'radio' || $nrow[type] == 'select' || $nrow[type] == 'checkbox'){
									$extr = array_merge(array('-1'=>'Any'),arraychange($value));
								}
...
function mymps_get_news_list($num=10,$catid='',$type=0,$leftjoin=true,$ifhot=''){
	global $city;
	$cat_limit  = empty($catid) ? '' : "AND a.catid IN(".get_cat_children($catid,'channel').")";
	$img_limit  = empty($type) ? '' : "AND a.imgpath != ''";
	$commend_limit = $ifhot == 1 ? " AND a.iscommend = '1'" : '';
	$orderby	= "ORDER BY a.id DESC";
	switch($leftjoin){
		case true:
			$query = $GLOBALS['db'] -> query("SELECT a.*,b.catname,b.htmlpath FROM `{$GLOBALS['db_mymps']}news` AS a LEFT JOIN `{$GLOBALS['db_mymps']}channel` AS b ON a.catid = b.catid WHERE 1 {$cat_limit} {$img_limit}{$city_limit} {$commend_limit} {$orderby} LIMIT 0,$num");
			while($row = $GLOBALS['db'] -> fetchRow($query)){
				$arr['id'] 			= $row['id'];
		...
		case false:
			$query = $GLOBALS['db'] -> query("SELECT a.* FROM `{$GLOBALS['db_mymps']}news` AS a WHERE 1 {$cat_limit} {$img_limit}{$city_limit} {$orderby} LIMIT 0,$num");
			while($row = $GLOBALS['db'] -> fetchRow($query)){
				$arr['id'] 			= $row['id'];
				$arr['title'] 		= $row['isbold'] == 1 ? '<strong>'.$row['title'].'</strong>' : $row['title'];
				$arr['title'] 		= $row['title'];
...
function mymps_get_member_list($num='',$level='',$orderby='',$if_corp='',$if_certify='',$ifindex='',$iflist=''){
	global $db,$db_mymps,$mymps_global,$city;
	$where 	 = $level ? " WHERE a.levelid >= '$level'":" WHERE a.levelid > '1'";
	$where	 .= $if_corp ? " AND a.if_corp = '$if_corp'" : " ";
	...
	while($row = $db -> fetchRow($do_mymps)){
		...
		$arr['prelogo']		= $row['prelogo'] ? $row['prelogo'] : '/images/nophoto.jpg';
		...
		$arr['introduce']	= substring(clear_html($row['introduce']),0,40);
		$arr['uri']      	= Rewrite('store',array('uid'=>$row['id']/*,'part'=>'album'*/));
		$member_list[]      = $arr;
	}
	return count($member_list)>0?$member_list:false;
}

/*
 *获得信息列表
 */
function mymps_get_info_list($num=10,$info_level='',$upgrade_type='',$userid='',$catid='',$certify='',$if_hot='',$sql='',$cityid='',$tel=''){
	...
	$where .= $catid	? ' AND a.catid IN ('.$catid.') ':'';
...
function mymps_get_focus($type='index',$num=5){
	global $city;
	$typename = $type == 'index' ? 'Site Homepage' : 'News Homepage';
	$city_limit = $city['cityid'] ? " AND cityid = '$city[cityid]'" : " AND cityid = '0'";
	$res = $GLOBALS['db']->getAll("SELECT id,image,url,words FROM `{$GLOBALS['db_mymps']}focus` WHERE typename = '$typename' {$city_limit} ORDER BY focusorder DESC LIMIT 0,$num");
	return $res;
}
...
function get_member_info($userid=''){
	$row = $GLOBALS['db'] -> getRow("SELECT userid,tname,per_certify,com_certify,jointime,prelogo,email,if_corp,credits,credit FROM `{$GLOBALS['db_mymps']}member` WHERE userid = '$userid'");
...
function chk_member_purview($purview)
{
	...
	$member  = get_member_group('',$s_uid);
	!in_array($purview,explode(',',$member['purviews'])) && write_msg("Members at your current level are not authorized to operate this column! <br />Please upgrade your member level first.","index.php?m=levelup");
}
...
/*填写信息密码以用于修改*/
function write_pwd($action='Edit'){
	global $post,$title,$cat,$part,$id,$db,$db_mymps,$mymps_global;
	$title = "Please Enter Administrator Code - ".$action."Post - ".$post[title];
	$nav_bar = '<a href="../information.php?id='.$id.'">'.$post[title].'</a> &raquo; Please Enter Administrator Code &raquo; '.$action.'Post</li>';
	$post['part'] = $part;
	globalassign();
...
function get_goods_children($catid)
{
...
function ifsiteopen(){
	...
	if($mymps_global['cfg_if_site_open'] != 1) {
		...
		exit('The site has been closed for the reason: <br><br /><b>'.$mymps_global['cfg_site_open_reason'].'</b>');
	}
}
...
function select_where($table,$name='parentid',$currentid='',$parentid=''){
	...
	if($parentid){
		...
		return $view ? "<select class='input' style='width:140px' id='$table' name=$name><option value=''>Please select  </option>$view</select>" : "";
...
function select_where_option($actionfile,$cityid,$areaid,$streetid){
	$actionfile = empty($actionfile) ? '/include/selectwhere.php' : $actionfile;
	$mymps .= '<select id=\'cityid\' name=\'cityid\'  onChange="choose_where(\'getarea\',this.options[this.selectedIndex].value,\'\',\'1\')"  class="input" style="width:140px!important" require="true" datatype="limit" msgid="District" msg="Please Select Source Sub-site">
...
';
	...
	$mymps .= $areaid ? 'choose_where("getstreet","'.$areaid.'","'.$streetid.'","0");' : '';
	$mymps .= '
//-->
</SCRIPT>
	';
...
function get_ip2city($ip){
	...
	if(is_array($all_cities = get_allcities())){
		foreach($all_cities as $k => $v){
			$v['cityname']=str_replace(array("City","District"," "),array("","",""),$v['cityname']);
...
function getbrowser() {  
    ...
    if (preg_match('/OmniWeb\/(v*)([^\s|;]+)/i', $agent, $regs)) {  
      ...
      $browser       = '(Internet Explorer ' .$browser_ver. ') Sogou';  
      $browser_ver   = '';  
