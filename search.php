...
if($keywords != '' && strlen($keywords) < 2) write_msg('The keyword you input is too short! Keywords to be input should not be less than 2 characters!');
...
$tel	  = isset($tel)		 ? checkhtml($_GET['tel'])	 : '';
$keywords = checkhtml($_GET['keywords']);
$keywords = str_replace('?','',$keywords);
$keywords = isset($keywords) ? trim(preg_replace("/(\s+)/",' ',$keywords)) : '';
$mod = isset($mod)	 ? trim(mhtmlspecialchars($mod)) : '';

if($keywords == 'Please inter keywords or category name') $keywords = $_GET['keywords'] = '';
...
if(!is_array($city)){
	$city = array('cityname'=>'Master Site','cityid'=>'0','cityname'=>$mymps_global['SiteUrl']);
}
...
switch($mod){
		...
		if($keywords != '' || $tel != ''){
			...
			$search['page_title'] = ($keywords != '' ? $keywords.' - ' : '').'信息搜索 - '.$mymps_global['SiteName'];
				...
				foreach($searchresult as $k => $val){
					...
					$result['poster']		= $val['ismember'] == 1 ? '<a target="black" href='.Rewrite('space',array("userid"=>$val["userid"])).$val["userid"].'</a>' : $val['contact_who'] ? $val['contact_who'] : 'Tourist';
	...
	case 'store':
		if($keywords != ''){
			$document_domain = str_replace('http://www.','',$mymps_global['SiteUrl']);
			$search['page_title'] = ($keywords != '' ? $keywords.' - ' : '').'Search For Seller - '.$mymps_global['SiteName'];
			if($keywords != ''){
				if(is_array($keyword = explode(' ',$keywords))){
					$or = '';
					$sqlkeywords = '';
				...
				if(is_array($res = page1($sql))){
					foreach($res as $key => $val){
						...
						$arr['tel']			= $val['tel'] ? $val['tel'] : 'You have not yet entered the phone number';
						$arr['address']		= $val['address'] ? $val['address'] : 'You have not yet entered the address';
	...
	case 'news':
		if($keywords != ''){
			!ifplugin('news') && exit('The news plug-in has either been disabled by the administrator or not been installed!');
			...
			$search['page_title'] = ($keywords != '' ? $keywords.' - ' : '').'Search For Site News - '.$mymps_global['SiteName'];
		}
	break;
	...
	case 'group':
		if($keywords != ''){
			...
			$search['page_title'] = ($keywords != '' ? $keywords.' - ' : '').'Search For Group Purchase Activity - '.$mymps_global['SiteName'];
		}
	break;
	...
	case 'coupon':
		if($keywords != ''){
			...
			$search['page_title'] = ($keywords != '' ? $keywords.' - ' : '').'Search For Coupon - '.$mymps_global['SiteName'];
		}
	break;
	...
	case 'goods':
		if($keywords != ''){
			...
			if($keywords != ''){
				...
				if($catid) $where .= " AND catid IN (".get_goods_children($catid).")";
				if($cityid) $where .= " AND cityid = '$cityid'";
			...
			$search['page_title'] = ($keywords != '' ? $keywords.' - ' : '').'Search For Product - '.$mymps_global['SiteName'];
		}
	break;
}
