...
!ifplugin(CURSCRIPT) && exit('The group purchase plug-in has either been disabled by the administrator or not been installed!');
...
if(!submit_check(CURSCRIPT.'_submit')){
	...
	if($id) {
		...
		if(!$group['groupid']) write_msg('This group purchase activity either doesn’t exist or have not yet passed the revision!',$mymps_global['SiteUrl']);
	...
	if(empty($id)) write_msg('The group purchase activity you wish to sign-up for does not exist!');
	if(empty($sname)) write_msg('Your name cannot be empty!');
	...
	write_msg('You have successfully signed-up, and we will contact you as soon as possible!',plugin_url(CURSCRIPT,array('id'=>$id)));
...
function get_group_location($cate_id=0,$str=''){
	...
	$location   = 'Current Location: <a href="'.($city['domain'] ? $city['domain'] : $GLOBALS['mymps_global']['SiteUrl']).'">'.$city['cityname'].$GLOBALS['mymps_global']['SiteName'].'</a>'.' <code>'.$raquo.'</code> '.' <a href="'.$city['domain'].plugin_url(CURSCRIPT,array('cate_id'=>0)).'">'.$city[cityname].'Group Purchase Activity</a>';
	$page_title = $city['cityname'].'Group Purchase Activity - '.$GLOBALS['mymps_global']['SiteName'];
